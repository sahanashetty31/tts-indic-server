...
def get_token(username, password):
    try:
        ...
        if response.status_code == 200:
            logger.info(f"Token request successful. Status code: {response.status_code}")
            token_response = response.json()
            return token_response.get("token", None)
        else:
            logger.error(f"Token request failed. Status code: {response.status_code}, {response.text}")
            return None
...
def get_audio(input_text, usecase_id, username, password):
    try:
        ...
        url = "https://slabstech-dhwani-server.hf.space/v1/audio/speech"
        headers = {
            "X-API-Key": token,
            "Content-Type": "application/json"
        }
